class Solution {
    public boolean exist(char[][] board, String word) {
        ...
        for(int i = 0 ; i < board.length ; i++){
            for(int j = 0 ; j < board[0].length ; j++){
                ...
                if(solve(board , word , set , i , j , 1 ))return true;
                set.remove(i + "," + j);}
            }
        }
        return false;
    ...
    public boolean solve(char[][] mat , String s ,HashSet<String> set , int i , int j , int index ){
        if(index == s.length()){
            return true;
        }
        ...
        if(i - 1 >= 0 && !set.contains((i - 1) + "," + j) && s.charAt(index) == mat[i - 1][j]){
            ...
            set.add((i - 1) + "," + j);
            if(solve(mat , s , set , i-1 , j , index + 1)) return true;
            set.remove((i - 1) + "," + j);
        }
        ...
        if(j - 1 >= 0 && !set.contains(i + "," + (j - 1)) && s.charAt(index) == mat[i][j -1]){
            set.add(i + "," + (j - 1));
            if(solve(mat , s , set , i , j -1 , index + 1)) return true;
            set.remove(i + "," + (j - 1));
        }
